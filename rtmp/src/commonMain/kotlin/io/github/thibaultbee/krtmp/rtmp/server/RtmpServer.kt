...
import io.github.thibaultbee.krtmp.rtmp.util.NetConnectionConnectCodeFailed
        ...
        when (command.name) {
            COMMAND_CONNECT_NAME -> {
                try {
                    ...
                    connection.writeAmfMessage(
                        Error(
                            command.messageStreamId,
                            1,
                            connection.settings.clock.nowInMs,
                            null,
                            Amf.encodeToAmfElement(
                                ...
                                NetConnectionResultInformation(
                                    ...
                                    code = NetConnectionConnectCodeFailed,
                                    description = "Connection failed: ${t.message}"
                                )
                            )
                        )
                    )
                ...
                try {
                    ...
                    connection.writeAmfMessage(
                        Command.OnStatus(
                            ...
                            connection.settings.clock.nowInMs,
                            Amf.encodeToAmfElement(
                                Command.OnStatus.OnStatusInformation.serializer(),
                                Command.OnStatus.OnStatusInformation(
            ...
            COMMAND_FCPUBLISH_NAME -> {
                try {
                    ...
                    connection.writeAmfMessage(
                        CommandOnFCPublish(command.transactionId, connection.settings.clock.nowInMs)
                    )
                } catch (t: Throwable) {
                    connection.writeAmfMessage(
                        Error(
