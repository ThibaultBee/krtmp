...
fun SetPeerBandwidth(timestamp: Int, payload: Buffer): SetPeerBandwidth =
    SetPeerBandwidth(
        timestamp,
        payload.readInt(),
...
class SetPeerBandwidth(timestamp: Int, windowSize: Int, limitType: LimitType) :
    Message(
    ...
    enum class LimitType(val value: Byte) {
        HARD(0x00),
        SOFT(0x01),
        DYNAMIC(0x02);

        companion object {
            fun from(findValue: Byte): LimitType = entries.first { it.value == findValue }
        }
    }
}
