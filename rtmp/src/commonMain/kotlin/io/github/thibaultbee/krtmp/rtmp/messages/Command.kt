...
import io.github.thibaultbee.krtmp.rtmp.messages.command.NetConnectionConnectResultInformation
...
import io.github.thibaultbee.krtmp.rtmp.util.NetConnectionConnectCodeSuccess
...
open class Command(
    ...
    override fun createMessage(amfVersion: AmfVersion): Message {
        val payload = AmfElementFactory.buildContainer(
            listOf(
                name, transactionId.toDouble(), commandObject
            ) + arguments
        )
    ...
    companion object {
        ...
        internal fun read(
            ...
            @Suppress("UNCHECKED_CAST") val name =
                (amfElementReader.read(payload) as AmfPrimitive<String>).value
            try {
                @Suppress("UNCHECKED_CAST") val transactionId =
    ...
    class Result(
        ...
        transactionId: Long,
        timestamp: Int,
        val resultObject: AmfElement? = null,
        val informationObject: AmfElement? = null,
    ...
    class Error(
        ...
        transactionId: Long,
        timestamp: Int,
        val errorObject: AmfElement? = null,
        val informationObject: AmfElement? = null,
        chunkStreamId: Int = ChunkStreamId.PROTOCOL_CONTROL.value
    ) : Command(
        chunkStreamId,
    ...
    class OnStatus(
        ...
        fun decodeInformation(): NetInformation {
            require(informationObject is AmfEcmaArray)
            val level = (informationObject["level"] as AmfString).value
            val code = (informationObject["code"] as AmfString).value
            val description = (informationObject["description"] as AmfString?)?.value
            return if (code == NetConnectionConnectCodeReconnect) {
                NetConnectionReconnectRequestInformation(
                    description = description ?: "",
                    tcUrl = (informationObject["tcUrl"] as AmfString).value
                )
            } else {
                OnStatusInformation(
        ...
        @Serializable
        open class OnStatusInformation(
        ...
        companion object {
            fun from(
                messageStreamId: Int, transactionId: Long, timestamp: Int, information: AmfElement
            ) = OnStatus(
                messageStreamId, transactionId, timestamp, information
            )
        }
    }
}
...
fun CommandNetConnectionResult(
    timestamp: Int,
    connectReplyObject: NetConnectionConnectResultObject = NetConnectionConnectResultObject.default,
    ...
    return Command.Result(
        ...
        Amf.encodeToAmfElement(
            NetConnectionConnectResultObject.serializer(), connectReplyObject
        ),
        Amf.encodeToAmfElement(
            NetConnectionConnectResultInformation.serializer(), NetConnectionResultInformation(
...
fun CommandConnect(
    transactionId: Long, timestamp: Int, connectObject: ConnectObject
): Command {
    ...
    return Command(
        ...
        transactionId,
        Amf.encodeToAmfElement(
            ConnectObject.serializer(), connectObject
        )
    )
}

fun CommandCreateStream(transactionId: Long, timestamp: Int) = Command(
    ChunkStreamId.PROTOCOL_CONTROL.value,
    MessageStreamId.PROTOCOL_CONTROL.value,
    timestamp,
...
fun CommandReleaseStream(
    transactionId: Long, timestamp: Int, streamKey: String
) = Command(
    ChunkStreamId.PROTOCOL_CONTROL.value,
    MessageStreamId.PROTOCOL_CONTROL.value,
    timestamp,
...
fun CommandCloseStream(transactionId: Long, timestamp: Int) = Command(
    ChunkStreamId.PROTOCOL_CONTROL.value,
    MessageStreamId.PROTOCOL_CONTROL.value,
    timestamp,
...
fun CommandDeleteStream(transactionId: Long, timestamp: Int, streamId: Int) = Command(
    ChunkStreamId.PROTOCOL_CONTROL.value,
    MessageStreamId.PROTOCOL_CONTROL.value,
    timestamp,
...
fun CommandPlay(timestamp: Int, streamName: String) = Command(
    ChunkStreamId.PROTOCOL_CONTROL.value,
    MessageStreamId.PROTOCOL_CONTROL.value,
    timestamp,
    COMMAND_PLAY_NAME,
...
fun CommandOnFCPublish(
    transactionId: Long, timestamp: Int
) = Command(
    ChunkStreamId.PROTOCOL_CONTROL.value,
    MessageStreamId.PROTOCOL_CONTROL.value,
    timestamp,
