...
internal constructor(
    ...
    naluSize + AVCC_HEADER_SIZE
)

/**
 * A [SizedRawSource] from a [Buffer].
 */
fun SizedRawSource(buffer: Buffer) = SizedRawSource(buffer, buffer.size.toInt())

/**
 * A [SizedRawSource] from a [ByteArray].
 */
fun SizedRawSource(array: ByteArray) = SizedRawSource(ByteArrayRawSource(array), array.size)

/**
 * A [RawSource] with a size.
 *
 * For AVC/H.264, HEVC/H.265 frame data (incl. parameter sets), the frames data must be in AVCC
 * format. AVCC means that each NALU is prefixed with a 4-byte size. To simplify the handling of
 * frame data, use [NaluRawSource] instead.
 *
 * @param source the [RawSource] to wrap.
 * @param byteCount the number of bytes of the [source]
 */
open class SizedRawSource(
    val source: RawSource,
    val byteCount: Int
) : RawSource {
    override fun readAtMostTo(sink: Buffer, byteCount: Long) = source.readAtMostTo(sink, byteCount)
